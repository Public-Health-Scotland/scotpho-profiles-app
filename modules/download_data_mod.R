...
# note this can be used to download reactive data (i.e. data that is filtered in some way based on user selection) or non-reactive (i.e. data that you read into your global script)
# there is also an optional arguments in the server function to specify which columns of data to include in the download 
# by default all columns are included unless you specify otherwise
...
download_data_btns_ui <- function(id) {
  ...
  shinyWidgets::dropdownButton(
    label = "Download data",icon = icon("download"), circle = FALSE,status = 'download', # menu
    tooltip = shinyWidgets::tooltipOptions(placement = "right", title = "Click to view available formats"), # tooltip
    shiny::downloadLink(ns("downloadCSV"), label = "as CSV", icon = NULL), # csv download button
    shiny::downloadLink(ns("downloadRDS"), label = "as RDS", icon = NULL), # rds download button
    shiny::downloadLink(ns("downloadJSON"), label = "as JSON", icon = NULL) # json download  button
  )
}
