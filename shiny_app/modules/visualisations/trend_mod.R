  ...
  moduleServer(id, function(input, output, session) {
    ...
    observeEvent(input$help, {
      showModal(modalDialog(
        ...
        tagList(
          layout_columns(
            paste0("The trend chart is designed to explore how a single indicator has changed over time for one or more geographical area.")),
          layout_columns(
            tags$img(src = "Trend_help_example_final.PNG"),
            paste0("First select an indicator. The backspace can be used to remove indicators and then indicators can be searched.
            Then add one or more geographical areas of any area type to the chart using the geography filters. 
                 ...
                 The trend chart is designed to explore how a single indicator has changed over time for one or more geographical area.
                 First select an indicator. The backspace can be used to remove indicators and then indicators can be searched.
                 Then add one or more geographical areas of any area type to the chart using the geography filters.
                 There may be some indicators where data is not available for the full time series or at a particular geography level.
                 Use the mouse to hover over a data point to see detailed information on its value, time period and area.
                 Confidence intervals (95%) can be added or removed from the chart. These are shown as shaded areas.
                 Confidence intervals give an indication of the precision of a rate or percentage. The width of a confidence interval is
                 related to sample size. Smaller geographies like intermediate zones often have wider intervals.
                 Display controls allow you to switch the graph from a measure (e.g. rate or percentage) to actual numbers (e.g. number
                 of births with a healthy birthweight).")))
      ))
    })
