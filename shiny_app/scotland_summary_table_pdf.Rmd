...
create_indicator_trend_line <- function(data) {
  ...
  geom_line(color = phs_colors(colourname = "phs-blue")) +
...
table <- df %>%
  select(indicator, measure, def_period, type_definition) %>%
  distinct() %>%
  mutate(ind = paste0(
...
col_names = c("indicator", "Scotland", "Trend")
...
table %>%
  ...
  # set column widths
   ...
   column_spec(2, width = "6em") %>% # scotland column


  # embed chart images on each row of final column
   kableExtra::column_spec(3, image = sprintf("%s/%s.png", graph_directory, table$id), color = "white") %>%
